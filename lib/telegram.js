...
const openai = new OpenAIApi(new Configuration({ apiKey: process.env.OPEN_API_KEY }));
...
export async function updateTelegramPosts() {
  ...
  try {
    return await Promise.all(posts.map((f) => collection.updateOne({ link: f.link }, { $set: f }, { upsert: true })))
      .then((records) => sendTwitterNotification(posts, records).catch(console.error))
      .catch(console.error);
  } catch (e) {
    return Promise.reject(e);
  }
  ...
  return openai
    .createChatCompletion({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Отвечай в формате json {title: string, summary: sting, tags: array, ad: int}. Придумай заголовок. Напиши кратко суть статьи в двух строках. Оцени вероятность того, что это рекламная статья в процентах. Предложи максимум 2 тега для статьи.
Статья:
${post.content.substring(0, MAX_CHARS)}`,
    ...
    .then((res) => {
      ...
      let meta = JSON.parse(res.data.choices[0].message.content || "{}");
      ...
      twitterClient
        ...
        .then(({ client, refreshToken }) => {
          ...
          return mongo
            .db("ityoutubers")
            .collection("config")
            .updateOne({ TWITTER_REFRESH_TOKEN: { $exists: true } }, { $set: { TWITTER_REFRESH_TOKEN: refreshToken } })
            .then(() => ({ client, refreshToken }));
        })
        .then(({ client, refreshToken }) => {
          const notifications = _(posts)
            ...
            .tap((filteredPosts) => console.log(`- ${filteredPosts.length} filtered posts`))
            ...
            .map((augmentedPost) =>
              augmentedPost
                ...
                .then(({ link, meta }) => {
                  ...
                  return client.v2
                    .tweet(fullTwitt.length > MAX_LENGTH ? `${meta.title}\n\n${link}` : fullTwitt)
                    .then((res) => console.log(`- Tweet sent: https://twitter.com/ityoutubers/status/${res.data.id}`))
                    .catch(console.error);
                })
            )
            .value();
...
function sendTelegramDigest() {
  ...
  const telegramSpinner = ora(`Sending Telegram digest since ${startDate}`).start();

  return mongo
    .db("ityoutubers")
    .collection("telegram")
    .find({ date: { $gt: startDate }, digested: { $exists: false }, meta: { $exists: true } }, { sort: { date: -1 } })
    .toArray()
    .then((posts) => {
      if (!posts.length) {
      ...
      return Promise.all(postsPages.map((postsPage, i) => 
        telegraf.telegram
        .sendMessage(process.env.TELEGRAM_CHANNEL_ID, i === 0 ? getDigestTitle() + "\n\n" + getDigest(postsPage): getDigest(postsPage), {
          parse_mode: "HTML",
...
if (currentModulePath === mainModulePath) {
  switch (process.argv[2]) {
    case "update":
      await updateTelegramPosts().then(() => setTimeout(() => process.exit(0), 15 * 60 * 1000));
      break;
    case "digest":
      await sendTelegramDigest().then(() => setTimeout(() => process.exit(0), 15 * 60 * 1000));
      break;
