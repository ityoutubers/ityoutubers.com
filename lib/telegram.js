...
${post.content.substring(0, MAX_CHARS)}`,
    ...
    .then((res) => {
      ...
      return { ...post, meta: JSON.parse(res.data.choices[0].message.content || "{}") };
    })
    .catch((e) => {
...
function getDigest(posts) {
  return `<b>Дайджест постов ${formatDateInRussian(new Date())}</b>\n\n
${posts
  .map(
    (post) =>
  ...
  return mongo
    ...
    .then((posts) => {
      ...
      return telegraf.telegram
        .sendMessage(process.env.TELEGRAM_CHANNEL_ID, getDigest(posts), {
          parse_mode: "HTML",
