...
export default function generateSlides(imagePaths, outputVideo) {
  ...
  imagePaths.forEach((imagePath) => {
    command.input(imagePath).inputOptions([`-loop 1`, `-t ${durationPerImage}`]);
  });

  return new Promise((resolve, reject) => {
    command
      .complexFilter(createFilterComplexScript(imagePaths.length, videoWidth, videoHeight))
      .output(outputVideo)
      .outputOptions([`-map [out]`, `-s:v ${videoWidth}x${videoHeight}`, `-f ${outputFormat}`])
      .on("start", (commandLine) => console.log("Starting video generation", commandLine))
  ...
  for (let i = 0; i < imageCount; i++) {
    ...
    script.push(
      `${
        ...
        videoHeight - 170
      }:w=150:h=150:color=black@0.5:t=fill,drawtext=text='${i + 1}':x=${textX}:y=${textY}:fontsize=100:fontcolor=white${
        outputs[i]
      }`
    );
  }
