...
export async function getYouTubeChannels() {
  ...
  await Promise.all(
    videosList.map((v) => db.collection("videos-dev").updateOne({ id: v.id }, { $set: v }, { upsert: true }))
  )
    .then(async (records) => {
      ...
      return [newVideos, res.data.items];
    })
    .then(
      // update videos with more info
      async ([newVideos, youtubeVideos]) => {
        await Promise.all(
          youtubeVideos.map((item) => db.collection("videos-dev").updateOne({ id: item.id }, { $set: { meta: item } }))
        ).catch(console.error);

        // return newVideos augmented with meta
        return newVideos.map((video) => ({ ...video, meta: youtubeVideos.find((item) => item.id === video.id) }));
      }
    )
    .then((videos) => videos.filter((video) => video.isMember))
    .then((videos) => sendVideoNotifications(videos))
    .catch(console.error);
      ...
      if (videos.length > 1) {
        ...
        const caption =
          videos
            .map(
              (video, i) =>
                `${i + 1}. <a href="https://youtu.be/${video.id}">${video.title}</a> (${duration(
                  video.meta.contentDetails?.duration
                )})`
            )
            .join("\n") +
          `\n\nАвтор: <a href="https://youtube.com/channel/${video.channelId}">${video.meta?.snippet.channelTitle}</a>`;
        ...
        await bot.telegram
          .sendPhoto(process.env.VIDEO_CHANNEL_ID, video.meta?.snippet.thumbnails.maxres.url, {
            caption:
              `<a href="https://youtu.be/${video.id}">${video.title}</a> (${duration(
                video.meta.contentDetails?.duration
              )})\n\n` +
              `Автор: <a href="https://youtube.com/channel/${video.channelId}">${video.meta?.snippet.channelTitle}</a>\n\n` +
