...
export async function getYouTubeChannels() {
  ...
  await Promise.all(
    videosList.map((v) => db.collection("videos").updateOne({ id: v.id }, { $set: v }, { upsert: true }))
  )
    .then(async (records) => {
    ...
    .then(
      ...
      async ([newVideos, youtubeVideos]) => {
        await Promise.all(
          youtubeVideos.map((item) => db.collection("videos").updateOne({ id: item.id }, { $set: { meta: item } }))
        ).catch(console.error);
